...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        if user_dict and any(key in user_dict for key in ['lprefix', 'lsuffix', 'lremname', 'ldump', 'yt_opt', 'bot_pm', 'media_group', 'equal_splits', 'split_size', 'rclone', 'thumb', 'as_doc']):
...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        text = BotTheme('MIRROR', NAME=name, RCLONE=rccmsg, DM=f"{dailyup} / {dailytlup}", MREMNAME=escape(mremname), MPREFIX=escape(mprefix),
